...
std::string RenderedShader::strVertexShader;
std::string RenderedShader::strPixelShader;
std::string RenderedShader::strPixelShaderUV;

void RenderedShader::initShader(bool bUV)
{
    if( bUV) {
    ...
    mixRfromR_Position = glGetUniformLocation(gProgram, "mixRfromR");
    mixRfromG_Position = glGetUniformLocation(gProgram, "mixRfromG");
    mixRfromB_Position = glGetUniformLocation(gProgram, "mixRfromB");

    mixBfromR_Position = glGetUniformLocation(gProgram, "mixBfromR");
    mixBfromG_Position = glGetUniformLocation(gProgram, "mixBfromG");
    mixBfromB_Position = glGetUniformLocation(gProgram, "mixBfromB");

}

void RenderedShader::finiShader()
{
...
GLuint RenderedShader::loadShader(GLenum shaderType, const char *pSource) {
...
GLuint RenderedShader::createProgram(const char *pVertexSource, const char *pFragmentSource) {
...
void RenderedShader::prepareShaderProgram() {
    glUseProgram(gProgram);
//    checkGlError("glUseProgram");
}

void RenderedShader::setPerspective( glm::mat4 &projectionMatrix) {
